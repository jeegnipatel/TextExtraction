...
pytesseract.pytesseract.tesseract_cmd = 'D:\\D\\Career\\Georgian\\Georgian\\AI Project\\Tesseract-OCR-20211006T033924Z-001\\Tesseract-OCR\\tesseract.exe'
...
def text_detector(image, frame_number):
    ...
    for (startX, startY, endX, endY) in boxes:
        ...
        textRecognized = textRecognized.replace("\f", "")
        textRecognized = textRecognized.replace("\n", "")
        textRecognized = textRecognized.replace("\r", "")

        print('Frame Number : ', frame_number)
        print(textRecognized)

        listing.append(textRecognized)
        cv2.rectangle(original, (startX, startY), (endX, endY), (0,
                      255, 0), 3)

    file1 = open('Extracted_data.txt', 'a')
    listToStr = ' '.join([str(elem) for elem in listing])
...
# reading test video
cap = cv2.VideoCapture('Video_text.mp4')
